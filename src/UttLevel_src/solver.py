        ...
        for i, (a_mix, a_tgt, v_tgt, clean_v_tgt, enroll_audio, speaker,) in enumerate(data_loader):              
            ...
            if state == "train":
                if 'spk' not in self.args.model_name.lower() and 'self' not in self.args.model_name.lower():
                    ...
                    if 'self' in self.args.model_name.lower() and 'spk' not in self.args.model_name.lower():
                        ...
                        if self.v_loss_weight !=0:
                            est_a_tgt1,_,_ = self.model(a_mix, v_tgt,a_zero,a_zero)
                            max_snr1 = cal_SISNR(a_tgt, est_a_tgt1)
                            sisnr_loss1 = self.v_loss_weight * (0 - torch.mean(max_snr1))
                            sisnr_loss1.backward()
                            loss += sisnr_loss1.data 
                            stage_1_loss += (0 - torch.mean(max_snr1)).data 
                    if 'spk' in self.args.model_name.lower():
                        # v+pre_enroll training 
                        sisnr_loss1 = 0
                        if self.v_loss_weight !=0:
                            # est_a_tgt1,_,_ = self.model(a_mix, v_tgt,a_zero,a_zero)  # for v_init setting 
                            est_a_tgt1,_,_ = self.model(a_mix, v_tgt,a_zero,enroll_audio)
                            max_snr1 = cal_SISNR(a_tgt, est_a_tgt1)
                            sisnr_loss1 = self.v_loss_weight * (0 - torch.mean(max_snr1))
                            sisnr_loss1.backward()
                            loss += sisnr_loss1.data 
                    ...
                    self_enroll = copy.deepcopy(est_a_tgt1.detach())
                    ...
                    if 'spkembmem' in self.args.model_name.lower():
                        pre_mem_bank = []
                        num_slots = random.randint(int(self.num_slot_range[0]),int(self.num_slot_range[1]))
                        for _ in range(num_slots):
                            T = self_enroll.shape[1]
                            segment_length = random.randint(1*self.args.sample_rate,T)
                            start_index = random.randint(0, T-segment_length)
                            segment = self_enroll[:,start_index: start_index + segment_length]
                            pre_mem_bank.append(segment)
                        random.shuffle(pre_mem_bank)
                        pre_enroll_input = pre_mem_bank
                    
                    sisnr_loss2 =0
                    if self.av_loss_weight != 0:
                        est_a_tgt2, _,_ = self.model(a_mix, v_tgt,self_enroll=self_enroll_input, pre_enroll = pre_enroll_input)       
                        max_snr2 = cal_SISNR(a_tgt, est_a_tgt2)
                        sisnr_loss2 = self.av_loss_weight * (0 - torch.mean(max_snr2))
                        sisnr_loss2.backward()
                        loss += sisnr_loss2.data 
                        stage_2_loss+=(0 - torch.mean(max_snr2)).data
                ...
                if 'spk' not in self.args.model_name.lower() and 'self' not in self.args.model_name.lower():
                    ...
                    if 'spk' in self.args.model_name.lower():
                        est_a_tgt1, _,_ = self.model(a_mix, clean_v_tgt,a_zero,enroll_audio)
                    self_enroll = copy.deepcopy(est_a_tgt1)
                    max_snr1 = cal_SISNR(a_tgt, est_a_tgt1)
