...
class av_convtasnet(nn.Module):
    def __init__(
        ...
        for key,param in self.spk_model.named_parameters():
            param.requires_grad = False
        self.self_att = CrossAttentionAVG(N)

    def forward(self, mixture, visual=None, self_enroll=None,pre_enroll=[]):
        with torch.no_grad():
        ...
        for audio in pre_enroll:
            with torch.no_grad():
                ...
                mem_bank.append(spk_emb.unsqueeze(1))
        mem_bank_tensor = torch.cat(mem_bank,dim=1) #B,slot,256

        T = mixture_w.shape[2]
        # Slot dim cross attn 
        spk_emb,attn_map = self.self_att(mem_bank_tensor)
        spk_emb = torch.repeat_interleave(spk_emb, repeats=T, dim=1) #B,T,C
        attn_map = torch.repeat_interleave(attn_map, repeats=T, dim=1) #B,T,slot
        ...
        return est_source, None, attn_map 
...
class TemporalConvNet(nn.Module):
    def __init__(self, N, B, H, P, X, R, C):
        ...
        self.linear_fusion = nn.Conv1d(N +256, N, kernel_size=1, stride=1, bias=False)
