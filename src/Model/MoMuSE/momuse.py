...
class momuse(nn.Module):
    def __init__(self, N, L, B, H, P, X, R, C, M,causal=True):
        super(momuse, self).__init__()
    ...
    def forward(self, mixture, visual,spk_emb,c=None, b=None):
        ...
        return est_source,new_spk_emb,None 
...
if __name__=='__main__':
    ...
    emb = torch.randn(1, 256).cuda()
    model.eval()
    est_source = model(mixture, visual, [emb])
    print(est_source[0].shape) # 8,16000
    print(est_source[1][0].shape) # 8,16000

    # from thop import profile
    # x_np = torch.randn(1, 16000).cuda()
    # v_np = torch.randn(1,25,112,112).cuda()
    # s_np = [torch.randn(1,256).cuda()]
    # flops, params = profile(model, inputs=(x_np, v_np,s_np))
