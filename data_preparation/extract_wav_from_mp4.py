...
def extract_wav_from_mp4(line):
    ...
    if not os.path.exists(audio_save_path):
        os.system(
            "ffmpeg -i %s -ar %d %s" % (video_from_path, args.sampling_rate, audio_save_path)
...
def main(args):
    if not os.path.exists(args.data_direc_mp4):
        print('!!!Video folder: ',args.data_direc_mp4,'not exist')
