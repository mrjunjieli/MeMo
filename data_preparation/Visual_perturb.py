...
def get_occluders(d, d_mask, state="test"):
    ...
    occluder_mask = cv2.resize(
        occluder_mask,
        (ori_occluder_img.shape[1], ori_occluder_img.shape[0]),
        interpolation=cv2.INTER_LANCZOS4,
    )

    try:
        occluder_img = cv2.bitwise_and(
            ori_occluder_img, ori_occluder_img, mask=occluder_mask
        )
    except Exception as e:
        print(e)
        return
    ...
    # cv2.imwrite('/mntnfs/lee_data1/wwu/TSE_MEMBANK/voxceleb2_occ_obj/org/org_img.png',occluder_img)
    occluder_img = cv2.resize(
        occluder_img, (size, size), interpolation=cv2.INTER_LANCZOS4
    )
    occluder_mask = cv2.resize(
        occluder_mask, (size, size), interpolation=cv2.INTER_LANCZOS4
    )
...
def occlude_sequence(
    d, d_mask, img_seq, landmarks, freq=1, bgr=False, data="LRS2"
):
    if freq == 1:
        ...
        for j in range(freq):
            ...
            offset_y = 15
            try:
                occ_len = random.randint(
                    int(len_global * 0.3), int(len_global * 0.5)
                )
...
def occlude_sequence_noise(img_seq, freq=1):
    ...
    if freq == 1:
        ...
        if prob < 0.5:
            ...
            raw_sequence = (
                random_noise(
                    raw_sequence, mode="gaussian", mean=0, var=var, clip=True
                )
                * 255
            )
        elif prob < 1.0:
            blur = torchvision.transforms.GaussianBlur(
                kernel_size=(7, 7), sigma=(0.1, 2.0)
            )
            raw_sequence = (
                blur(torch.tensor(raw_sequence).permute(0, 3, 1, 2))
                .permute(0, 2, 3, 1)
                .numpy()
            )
        else:
...
def landmarks_interpolate(landmarks):
    ...
    if valid_frames_idx:
        ...
        landmarks[valid_frames_idx[-1] :] = [
            landmarks[valid_frames_idx[-1]]
        ] * (len(landmarks) - valid_frames_idx[-1])
    valid_frames_idx = [idx for idx, _ in enumerate(landmarks) if _ is not None]
    assert len(valid_frames_idx) == len(
        landmarks
    ), "not every frame has landmark"
    return landmarks
...
def preprocess(video_pathname, landmarks_pathname):
    ...
    if (
        ...
        or len(preprocessed_landmarks) < _window_margin
    ):
...
def write_video(video, filename):
    ...
    output = cv2.VideoWriter(
        filename, fourcc, 25, (video.shape[1], video.shape[2])
    )
