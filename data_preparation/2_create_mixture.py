...
def main(args):
    ...
    for line in tqdm.tqdm(mixture_data_list, desc="Generating audio mixtures"):
        ...
        # read target audio
        tgt_audio, sr = audioread(
            args.audio_data_direc
            + data[1]
            + "/"
            + data[2]
            + "/"
        ...
        if sr != sampling_rate:
            tgt_audio = librosa.resample(
                tgt_audio, orig_sr=sr, target_sr=sampling_rate
            )
        ...
        for c in range(1, args.C):
            ...
            if sr != sampling_rate:
                audio = librosa.resample(
                    audio, orig_sr=sr, target_sr=sampling_rate
                )
            snr = float(data[c * 4 + 4])

            # truncate long audio with short audio in the mixture
            if "test" not in save_direc:
                tgt_audio, intervention_audio, mix_audio, _ = (
                    segmental_snr_mixer(
                        tgt_audio,
                        intervention_audio,
                        snr,
                        length=int(args.max_length * args.sampling_rate),
