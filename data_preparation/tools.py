...
def segmental_snr_mixer(
    ...
    if length != None:
        ...
        if len(noise) > length:
            noise = noise[0:length]
        if len(noise) < length:
            noise = np.append(noise, np.zeros(length - len(noise)))
        if len(clean) < length:
            clean = np.append(clean, np.zeros(length - len(clean)))
    if min_option == False and length == None:
